...
bool MPU6050::read(imu_data& out){
    int16_t accel[3];
...
void MPU6050::read_raw_accel(int16_t accel[3]){
    uint8_t buffer[6];
    platform.i2cRead(MPU6050_ACCEL_OUT, buffer, 6);
...
bool MPU6050::get_id(){
    uint8_t buffer[1];
    platform.i2cRead(MPU6050_ID_LOC, buffer, 1);
...
void MPU6050::set_range(mpu6050_accel_range_t accel_range_in){
    uint8_t buf[1];
    buf[0] = (accel_range_in << 3);
    platform.i2cRead(MPU6050_ACCEL_CONFIG, buf, 1);
...
void MPU6050::update_range(){
    uint8_t buf[1];
    platform.i2cRead(MPU6050_ACCEL_OUT, buf, 1);
